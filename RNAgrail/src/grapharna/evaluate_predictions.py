...
def parse_args():
    ...
    args.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the predictions directory (*.trafl files)",
    )
    args.add_argument(
        "--templates-path",
        type=str,
        default=None,
        help="Path to the pdb templates directory",
    )
    args.add_argument(
        "--targets-path",
        type=str,
        default=None,
        help="Path to the pdb targets directory",
    )
    args.add_argument(
        "--output-name", type=str, default="rmsd.csv", help="Name of the output file"
    )
    args.add_argument(
        "--sim_rna", type=str, default=None, help="Path to the simRNA executable"
    )
    args.add_argument(
        "--arena_path", type=str, default=None, help="Path to the arena executable"
    )
    args.add_argument(
...
def generate_pdbs_from_trafl(
    trafl_path,
    targets_path,
    sim_rna_path,
    overwrite=False,
    pdb_postfix=".pdb",
    out_postfix="-000001_AA.pdb",
):
    ...
    for t in tqdm(trafls):
        ...
        if os.path.exists(f"{targets_path}/{pdb_name}"):
            os.system(
                f"./run_SimRNA_trafl_to_pdb.sh {sim_rna_path} {targets_path}/{pdb_name} {trafl_path}/{t}"
            )
        else:
            print(
                f"Skipping {t} as the target pdb file: {targets_path}/{pdb_name} does not exist"
            )
...
def generate_pdbs_AA(samples_path, arena_path, overwrite=False, out_postfix="_AA.pdb"):
    ...
    samples = [
        ...
        if sample.endswith(".pdb") and not sample.endswith(out_postfix)
    ]
    for s in tqdm(samples):
        ...
        if not overwrite and os.path.exists(os.path.join(samples_path, out_name)):
            continue
        os.system(
            f"./run_ARENA_pdb_to_aa.sh {arena_path} {samples_path}/{s} {samples_path}/{out_name}"
        )
...
def get_inf(s_pred, s_gt):
    assert len(s_pred) == len(s_gt), ValueError(
        "Length of the predicted and ground truth sequences should be the same"
    )
...
def superimpose_pdbs(
    trafl_path, targets_path, out_postfix="-000001_AA.pdb", method: str = "pymol"
):
    ...
    for pdb in tqdm(pdbs):
        ...
        if os.path.exists(f"{targets_path}/{pdb_name}"):
            ...
            print(
                f"Skipping {pdb} as the target pdb file: {targets_path}/{pdb_name} does not exist"
            )
    return outs
...
def align_biopython(trafl_path, targets_path, pdb, pdb_name):
    ...
    try:
        sup.set_atoms(
            ref_atoms, atoms
        )  # if there is a missing P atom in some chain, then is should be removed from prediction to do superposition
...
def main():
    ...
    if args.sim_rna is not None:
        generate_pdbs_from_trafl(
            args.preds_path, args.templates_path, args.sim_rna, args.overwrite
        )  # , out_postfix=out_postfix, pdb_postfix=".pdb")
    elif args.arena_path is not None:
        generate_pdbs_AA(
            args.preds_path, args.arena_path, args.overwrite, out_postfix="_AA.pdb"
        )
    print("Superimposing...")
    outs = superimpose_pdbs(
        args.preds_path, args.targets_path, out_postfix="_AA.pdb", method="pymol"
    )
    print("Results:")
    ...
    print(df.head())
    print(
        f"Mean RMSD: {df['rms'].mean()}, Std: {df['rms'].std()}, Median RMSD: {df['rms'].median()}"
    )
    print(
        f"Mean eRMSD: {df['ermsd'].mean()}, Std: {df['ermsd'].std()}, Median eRMSD: {df['ermsd'].median()}"
    )
    print(
        f"Mean INF: {df['inf'].mean()}, Std: {df['inf'].std()}, Median INF: {df['inf'].median()}"
    )
