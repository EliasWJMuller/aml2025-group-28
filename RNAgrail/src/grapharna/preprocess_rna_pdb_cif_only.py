...
from grapharna.constants import (
    ATOM_ELEMENTS,
    ATOM_TYPES,
    COARSE_GRAIN_MAP,
    DOT_CLOSINGS_MAP,
    ...
    for model in structure:
        for chain in model:
            for residue in chain:
                ...
                if (
                    residue.id[0].startswith("H_")
                    or residue.get_resname() not in RESIDUES.keys()
                    or res_id >= len(seq_segments[0])
                ):  # sometimes there is
                    continue
                ...
                if (
                    sum(res_coords_update) >= 3
                ):  # if at least 3 atoms are present, consider the residue as present
                    coords_in_residue[res_id] = res_coords
                    coords_updated[res_id] = res_coords_update
                    res_id += 1

    return (
        coords_in_residue.reshape((-1, 3)),
        atoms_elements,
        atoms_names,
        residues_names,
        p_missing,
...
def get_coarse_grain_mask(symbols, residues):
    ...
    mask = [
        True if atom in coars_atoms else False
        for atom, coars_atoms in zip(symbols, coarse_atoms)
    ]
    return np.array(mask)


def get_edges_in_COO(
    data: dict,
    seq_segments: list[str],
    p_missing: list[bool],
    bpseq: list[tuple[int, int]] = None,
):
    ...
    for index in indicies:
        if p_missing[
            index
        ]:  # the missing P can occur only in the first residue of the segment
            ...
            nodes_indecies = np.concatenate(
                [
                    nodes_indecies[: index * 5],
                    np.array([nodes_indecies[index * 5]]),
                    nodes_indecies[index * 5 :],
                ]
            )  # add "fake" P atom, with the same node index (that will be filtered out later).
            added += 1

    combined = combined.reshape((-1, 5, 5))
    nodes_indecies = nodes_indecies.reshape((-1, 5))
    comb_arg_max = np.argmax(
        combined, axis=2
    )  # sometimes the order of atoms is 0,1,2,3,4, and sometimes it's different
    for res_ni, res_arg_max in zip(
        nodes_indecies, comb_arg_max
    ):  # create edges in each residue
        for i, j in RESIDUE_CONNECTION_GRAPH:
            edge = [
                res_ni[np.where(res_arg_max == i)[0]],
                res_ni[np.where(res_arg_max == j)[0]],
            ]
            if (
                edge[0] == edge[1]
            ):  # remove self loops, effect of adding missing P atoms
                continue
            edges.append(edge)
    ...
    for i in range(1, len(nodes_indecies)):
        if i in segments_lengs:
            continue
        prev_c4p = nodes_indecies[i - 1][
            np.where(comb_arg_max[i - 1] == 1)[0]
        ]  # C4' atom index in previous residue
        curr_p = nodes_indecies[i][
            np.where(comb_arg_max[i] == 0)[0]
        ]  # P atom index in current residue
...
def bpseq_to_res_ids(bpseq):
    ...
    bpseq = [
        (int(x[0]) - 1, int(x[2]) - 1)
        for x in bpseq
        if int(x[2]) != 0 and int(x[0]) < int(x[2])
    ]  # -1, because the indices in bpseq are 1-based, and we need 0-based (numpy indicies)
    return bpseq
...
def dot_to_bpseq(dot):
    ...
    for i, x in enumerate(dot_line):
        assert x in DOT_OPENINGS + list(DOT_CLOSINGS_MAP.keys()) + ["."], (
            f"Invalid character in dotbracket: {x}"
        )
...
def process_rna_file(
    rna_file, seq_segments, file_3d_type, sampling, save_dir_full, name, res_pairs
):
    if sampling:
        (
            rna_coords,
    ...
    )  # keep only C, N, O, P atoms
    res_indices = set(
        [i for i, x in enumerate(residues_names) if x in RESIDUES.keys()]
    )  # keep only A, G, U, C residues
    ...
    rna_x = np.array(
        [ATOM_TYPES[x] for x in elements]
    )  # Convert atomic numbers to types
    residues_x = np.array(
        [RESIDUES[x] for x in residues_names]
    )  # Convert residues to types
    ...
    try:
        edges, edge_type = get_edges_in_COO(
    ...
    with open(
        os.path.join(save_dir_full, name.replace(file_3d_type, ".pkl")), "wb"
    ) as f:
        pickle.dump(data, f)
...
def main():
    parser = argparse.ArgumentParser(
        description="Preprocess PDB/CIF files for RNA graph generation."
    )
    parser.add_argument(
        "--pdb_dir", type=str, required=True, help="Directory containing PDB/CIF files."
    )
    parser.add_argument(
        "--save_dir",
        type=str,
